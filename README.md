...
- 食事の記録はあすけん等のアプリで行い、VitaMorphはHealth Connect経由で栄養を読み取るだけです(v1.1でアプリ内の食事入力機能は削除)。
- 読み取りのみのため外部通信は行いません。

### 連携と保存

- Android Health Connectから歩数、活動カロリー、運動時間、栄養(あすけん等の記録)を読み取り
- あすけんのカロリー・PFC目標を初回設定
- トレーナー名の設定と変更
- SharedPreferencesとRoomによる端末内保存(世代・機嫌・絆・継承ポイントなど)
- 実データなしで全サイクルを確認できるデモモード

## プライバシー

- 健康・栄養データは端末内で進化判定と表示に使い、外部サーバーへ送信しません。
- 食事はあすけんで記録し、VitaMorphはHealth Connect経由で読み取るだけです。ゲームはネットワーク権限を要求せず、外部通信は行いません。
...
アプリアイコンはアダプティブアイコン(ベクター)です。ランチャーのマスク(円・角丸など)で表示を確認でき、テーマアイコン(Android 13+)にも対応しています。
