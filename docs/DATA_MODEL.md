...
## Recipe / RecipeItem(レシピ材料保存、v0.11)

v0.11で `recipe` と `recipe_item` の2テーブルとして確定(DBバージョン9、COMPLETION_PLAN T5)。
よく食べる組み合わせを材料付きで保存・閲覧・削除できるようにする。

```
recipe
  recipeId          主キー(自動採番)
  name              レシピ名
  createdAt         保存時刻(epoch millis)

recipe_item
  itemId            主キー(自動採番)
  recipeId          親レシピID(index_recipe_item_recipeId でインデックス)
  name              材料名
  amount            数量
  amountUnit        数量単位
  calories / proteinGrams / fatGrams / carbsGrams   数量換算後の栄養値
```

- 外部キー制約は張らず(既存スキーマの方針に合わせる)、削除時にDAOで `recipe_item`→`recipe` の順に消す。
- 合計栄養(`Recipe.totalCalories` ほか)は材料の総和として算出する。
- レシピ保存時に合計を「1食」の自作食品(`custom_food`)としても同時保存し、従来どおりカタログから1タップ記録できるようにする。
- 一覧から朝/昼/夕/間食の区分を選ぶと、合計栄養を1件(1食)の `food_entry` として記録する(HC書き込みも通常の記録と同じ)。
- 全リセット時に他テーブルとあわせてクリアする。
- 組み立ては純ロジック `RecipeFactory.fromParts`(各材料を `FoodCatalogItem.scaledTo` で数量換算)。
