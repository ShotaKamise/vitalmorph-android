...
### T6: ドキュメント整備とロスター更新 — [x] 完了(2026-07-12)
...
### T7: v1.0.0リリース準備 — [x] 完了(2026-07-12)

- バージョン 1.0.0 / versionCode 12 へバンプ済み。
- T1〜T6完了を確認。デモモードでの4週進化サイクル確認手順はREADMEへ記載済み(T6)。
- 署名済みAPKの公開: タグ `v1.0.0` をプッシュするとreleaseワークフローが実行される(署名シークレットが必要なためタグ付けはユーザー操作)。

## 完了

全タスク完了(2026-07-12)。以降の作業はユーザーと相談して決める。
